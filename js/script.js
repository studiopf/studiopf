...
function changelanguegalerie() {
    ...
    contenuPrincipal.innerHTML = galerieHTML;
    initializeGalerie(); // Réinitialiser la galerie après mise à jour
}
/***********************
...
function setLangue(lang) {
    ...
    // Recharger la page actuelle avec la nouvelle langue
    loadPage(urlencours);
}
...
function changelanguepeinturecommission() {
    ...
    if (langueselect === "english") {
        contentHTML = `
            <h1>Tabletop & Display Commission Painting 🎲</h1>
            <!-- Contenu en anglais complet comme dans l'original -->
            <!-- ... (insérez le contenu complet de l'original pour english) ... -->
        `;
    } else if (langueselect === "spanish") {
        contentHTML = `
            <h1>Pintura por Encargo Tabletop y Vitrina 🎲</h1>
            <!-- Contenu en espagnol complet comme dans l'original -->
            <!-- ... (insérez le contenu complet de l'original pour spanish) ... -->
        `;
    } else { // french
        contentHTML = `
            <h1>Peinture sur Commission TableTop et Vitrine 🎲</h1>
            <!-- Contenu en français complet comme dans l'original (truncated dans le prompt, mais assumez complet) -->
            <!-- ... (insérez le contenu complet de l'original pour french) ... -->
        `;
    }
    contenuPrincipal.innerHTML = contentHTML;
    initializeCardToggle(); // Réinitialiser
}
/*****************************************
 * Page index
 *****************************************/
...
function changelangueindex() {
    ...
    contenuPrincipal.innerHTML = contentHTML;
    initializeCardToggle(); // Réinitialiser les toggles après mise à jour
}
...
// Fonction loadPage corrigée pour charger le contenu et appliquer la langue
function loadPage(page) {
    urlencours = page;
    const contenuPrincipal = $('#contenu-principal');
    if (!contenuPrincipal.length) return;

    contenuPrincipal.load(page + ' #contenu-principal', function(response, status, xhr) {
        if (status === "error") {
            console.error("Erreur de chargement de " + page + ": " + xhr.status + " " + xhr.statusText);
            return;
        }
        // Après chargement, appliquer la langue et initialiser
        changelanguemenu();
        changelangueinfo();
        if (page === 'index.html') changelangueindex();
        if (page === 'galerie.html') changelanguegalerie();
        if (page === 'peinturecommission.html') changelanguepeinturecommission();
        // Ajoutez pour d'autres pages comme simulateur_devis.html, etc.
        initializeCardToggle();
        updateAge();
        scrollToTop();
        // Réinitialiser d'autres composants comme le carrousel, formulaire, etc.
        initializeCarousel(); // Si nécessaire
        initializeChat(); // Si chat sur la page
        initializeFormCalculations(); // Pour simulateur
        initializeFormationForm(); // Pour formation
    });
}
